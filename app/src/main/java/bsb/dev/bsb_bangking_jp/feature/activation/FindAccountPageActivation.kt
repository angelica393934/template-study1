...
@Composable
fun FindAccountPageActivation(
    viewModel: ActivationViewModel,
    onBackClick: () -> Unit,
    onNavigateToIdPengguna: () -> Unit,
) {
    val uiState by viewModel.uiState.collectAsStateWithLifecycle()
    val toastState = LocalToastState.current
    val loadingOverlay = LocalLoadingOverlay.current

    var rekeningInput by remember { mutableStateOf(uiState.atmCardNo) }

    LaunchedEffect(Unit) {
        viewModel.navEvent.collect { event ->
            if (event is ActivationNavEvent.ToIdPage) onNavigateToIdPengguna()
        }
    }
    LaunchedEffect(Unit) {
